...
export default function MapScreen() {
  ...
  // Copy all place names to clipboard
  const copyAllPlaces = async () => {
    const validPlaces = places.filter(p => p.place_name);
    
    if (validPlaces.length === 0) {
      Alert.alert('No Places', 'No places to copy.');
      return;
    }

    const placeList = validPlaces.map((place, i) => 
      `${i + 1}. ${place.place_name}${place.address_search_query ? ` - ${place.address_search_query}` : ''}`
    ).join('\n');

    await Clipboard.setStringAsync(placeList);
    Alert.alert('✅ Copied!', `${validPlaces.length} places copied to clipboard`);
  };
      ...
      <View style={styles.header}>
        ...
        {places.length > 0 && (
          <TouchableOpacity
            ...
            onPress={copyAllPlaces}
            activeOpacity={0.8}
          >
            <Text style={styles.copyAllIcon}>📋</Text>
            <Text style={styles.copyAllText}>Copy All Places</Text>
          </TouchableOpacity>
        )}
      </View>
